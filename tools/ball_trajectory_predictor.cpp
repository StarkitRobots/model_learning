...
int main(int argc, char** argv)
{
  ...
  for (size_t seq_id = 0; seq_id < sequences.size(); seq_id++)
  {
    ...
    std::uniform_real_distribution<double> start_distribution(config.min_time_to_start, config.max_time_to_start);
    ...
    for (const auto& pair : seq_by_type)
    {
      ...
      {
        csv_file << seq_id << "," << seq_name << "," << pos(0) << "," << pos(1) << "," << pos(2) << std::endl;
      }
    }
  }
}
