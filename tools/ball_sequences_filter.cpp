...
int main(int argc, char ** argv) {
  ...
  if (argc < 4) {
    std::cerr << "Usage: " << argv[0]
              << "<position_sequence_reader.json> <speed_estimator.json> <trajectory_files.csv>" << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  for (const std::string & traj_path : traj_paths) {
    ...
    for (size_t traj_idx = 0; traj_idx < sequences.size(); traj_idx++) {
      ...
      for (const Eigen::Vector3d & entry : seq.timed_positions) {
        ...
        std::cout << log_idx << "," << traj_idx << "," << entry[0] << "," << entry[1] << "," << entry[2] << "," << speed[0]  << "," << speed[1]  << std::endl;
      }
    }
    log_idx++;
  }
