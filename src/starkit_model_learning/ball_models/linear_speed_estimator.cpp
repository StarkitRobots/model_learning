...
{
LinearSpeedEstimator::LinearSpeedEstimator() : SpeedEstimator(), window_duration(0.5), max_ball_speed(4)
{
}
...
Eigen::VectorXd LinearSpeedEstimator::predictObservation(const starkit_model_learning::Input& raw_input,
...
void LinearSpeedEstimator::fromJson(const Json::Value& v, const std::string& dir_name)
{
  ...
  starkit_utils::tryRead(v, "noise", &noise);
  starkit_utils::tryRead(v, "window_duration", &window_duration);
}
