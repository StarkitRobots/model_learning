...
ModelLearner::ModelLearner(std::unique_ptr<Model> model_, std::unique_ptr<ModelPrior> prior_,
                           ...
                           std::unique_ptr<starkit_bbo::Optimizer> optimizer_, const std::set<int> trainable_indices_)
                                                   ...
                                                   std::default_random_engine* engine)
{
  ...
  Result result;
  starkit_bbo::Optimizer::RewardFunc reward_function = [this, &training_set](const Eigen::VectorXd& parameters,
                                                                            std::default_random_engine* engine) {
...
Json::Value ModelLearner::toJson() const
{
  ...
  v["trainable_indices"] = starkit_utils::set2Json(trainable_indices);
  return v;
}
...
void ModelLearner::fromJson(const Json::Value& v, const std::string& dir_name)
{
  ...
  starkit_utils::tryReadSet<int>(v, "trainable_indices", &trainable_indices);
  std::vector<std::string> indices_names;
  starkit_utils::tryReadVector<std::string>(v, "indices_names", &indices_names);
