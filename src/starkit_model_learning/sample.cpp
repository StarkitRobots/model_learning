...
{
Sample::Sample(std::unique_ptr<Input> input_, const Eigen::VectorXd& observation_)
...
DataSet splitSamples(const SampleVector& samples, int nb_validation_samples, std::default_random_engine* engine)
{
  ...
  validation_indices = starkit_random::getKDistinctFromN(nb_validation_samples, samples.size(), engine);
