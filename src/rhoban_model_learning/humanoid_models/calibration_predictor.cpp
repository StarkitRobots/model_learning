...
Eigen::VectorXd CalibrationPredictor::predictObservation(const Input & raw_input,
  ...
  if (!success && pixel(0) == 0 && pixel(1) == 0) {
    ...
    Eigen::Vector3d viewVectorInWorld =
      model.get().cameraPixelToViewVector(camera_model, camera_model.getCenter());
...
double CalibrationPredictor::computeLogLikelihood(const Sample & sample,
                                                  const Model & model,
                                                  std::default_random_engine * engine) const {
