...
void CameraModel::setParameters(const Eigen::VectorXd & new_params) {
  int idx = 0;
  model.setFocal(new_params.segment(idx,2)); idx += 2;
  model.setCenter(new_params.segment(idx,2)); idx += 2;
  model.setDistortion(new_params.segment(idx,5)); idx += 5;
}
...
void CameraModel::fromJson(const Json::Value & json_value,
